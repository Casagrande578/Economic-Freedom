...
def transform_csv(input_file, output_file):
    ...
    # Add this after loading df_metrics (around line 9)
    print("Available columns:", df_metrics.columns.tolist())
    print("Sample of Quartile values:", df_metrics['Quartile'].head())
    ...
    df['Quartile'] = df['Quartile'].apply(
        lambda x: "Most Free" if pd.notnull(x) and x == 1
        else "2nd Quartile" if pd.notnull(x) and x == 2
        else "3rd Quartile" if pd.notnull(x) and x == 3
        else "Least Free" if pd.notnull(x)
        else "N/A"
    )
    ...
    new_df['Area'] = new_df['Research Code'].apply(get_area)
    
    new_df['Área / Area'] = new_df['Research Code'].apply(lambda x: get_area(x))
    
    new_df['Quartil / Quartile'] = df['Quartile']
    ...
    new_df['Indice / Index - Discrete'] = pd.to_numeric(new_df['Indice / Index - Discrete'], errors='coerce')

    # Add debug print to see the values
    print("Sample of Indice / Index - Discrete values:", new_df['Indice / Index - Discrete'].head())
    print("Data type:", new_df['Indice / Index - Discrete'].dtype)

    new_df['Quartil - Eco Free'] = new_df['Indice / Index - Discrete'].apply(
        lambda x: "Most Free" if pd.notnull(x) and x > 7.45
        else "2nd Quartile" if pd.notnull(x) and x >= 6.90
        else "3rd Quartile" if pd.notnull(x) and x >= 6.29
        else "Least Free" if pd.notnull(x)
        else "N/A"
    )

    # Add debug print to see the results
    print("Sample of Quartil - Eco Free values:", new_df['Quartil - Eco Free'].head())
